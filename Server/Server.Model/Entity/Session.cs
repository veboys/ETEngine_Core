...
namespace Model
{
    public sealed class Session : Entity
    {
        ...
        private void Run(ushort opcode, byte[] messageBytes)
        {
            ...
            Log.Debug($"recv message: opcode={opcode}, compressed={isCompressed}");
        ...
        private void RunDecompressedBytes(ushort opcode, byte[] messageBytes, int offset)
        {
            ...
            Log.Debug($"try deserialize message: messageType={messageType.FullName}");
            Log.Debug($"{messageBytes.ToHex()}");
        ...
        private void SendMessage(object message)
        {
            ...
            Log.Debug($"try send message: opcode={opcode}, messageType={message.GetType().FullName}, compressed={flag == 0x80}");
            Log.Debug($"{messageBytes.ToHex()}");
